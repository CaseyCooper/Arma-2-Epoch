...
_baserunover11 = createVehicle ["Land_Dirthump01", [(_position select 0) + 8, (_position select 1) + 0.2,-0.02],[], 0, "CAN_COLLIDE"];
...
_baserunover31 = createVehicle ["Land_Fire_barrel_burning", [(_position select 0) + 2, (_position select 1) - 9,-0.02],[], 0, "CAN_COLLIDE"];
	...
	if(_complete) then {
		...
		if (_randomCrate >= 80 && _randomCrate <= 100) then {
			[_crate,9,5,[15,crate_items_crop_raider]] call dynamic_crate;
		};			
	};
