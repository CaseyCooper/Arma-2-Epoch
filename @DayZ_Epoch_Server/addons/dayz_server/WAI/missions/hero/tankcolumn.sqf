...
_baserunover13 = createVehicle ["Land_Fire_barrel_burning",[(_position select 0) - 9, (_position select 1) - 1,-0.02],[], 0, "CAN_COLLIDE"];
	...
	//Troops
	[[(_position select 0) - 7, (_position select 1) - 10, 0],4,"Hard",["Random","AT"],4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 16, (_position select 1) - 5, 0],4,"Hard","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 4, (_position select 1) + 18, 0],4,"Hard","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;

	//Humvee Patrol
	[[(_position select 0) + 22, (_position select 1) + 32, 0],[(_position select 0) + 15, (_position select 1) - 33, 0],50,2,new_ai_vehicle_random_bandit,"Hard","Bandit","Bandit",_mission] call vehicle_patrol;
	...
	if(_complete) then {
		...
		if (_randomCrate >= 60 && _randomCrate < 80) then {
			[_crate,6,[8,crate_tools_sniper],[25,crate_items_buildables]] call dynamic_crate;
		};	
