...
_baserunover23 = createVehicle ["UralWreck", [(_position select 0) - 41.3, (_position select 1) - 26,-0.02],[], 0, "CAN_COLLIDE"];
	...
	[[(_position select 0) + 12, (_position select 1) + 42.5, 0],_num,"extreme",["random","at"],4,"random","Bandit","random",["bandit",150],_mission] call spawn_group;
	[[(_position select 0) + 11, (_position select 1) + 41, 0],4,"hard","random",4,"random","Bandit","random","bandit",_mission] call spawn_group;
	[[(_position select 0) - 12, (_position select 1) - 43, 0],4,"random","random",4,"random","Bandit","random","bandit",_mission] call spawn_group;
	[[(_position select 0) - 13, (_position select 1) - 43, 0],4,"random","random",4,"random","Bandit","random","bandit",_mission] call spawn_group;
	...
	if(_complete) then {
		...
		if (_randomCrate >= 20 && _randomCrate < 40) then {
			[_crate,14,[5,crate_tools_sniper],[3,crate_items_high_value]] call dynamic_crate;
		};
