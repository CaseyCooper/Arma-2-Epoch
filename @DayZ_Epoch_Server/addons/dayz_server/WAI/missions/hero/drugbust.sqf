...
_baserunover23 = createVehicle ["MAP_t_salix2s", [(_position select 0) - 24, (_position select 1) - 0.01,-0.01],[], 0, "CAN_COLLIDE"];
	...
	//Troops
	[[(_position select 0) - 12, (_position select 1) - 15,0],4,"Medium",["Random","AT"],4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 12, (_position select 1) + 15,0],4,"Medium","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[_position select 0,_position select 1,0],4,"Medium","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	...
	if(_complete) then {
		...
		if (_randomCrate < 20) then {
			[_crate,10,5,25] call dynamic_crate;
		};
		if (_randomCrate >= 20 && _randomCrate < 40) then {
			[_crate,14,[4,crate_tools_sniper],[2,crate_items_high_value]] call dynamic_crate;
		};
		if (_randomCrate >= 40 && _randomCrate < 60) then {
			[_crate,0,0,[50,crate_items_medical]] call dynamic_crate;
		};
		if (_randomCrate >= 60 && _randomCrate < 80) then {
			[_crate,[6,ai_wep_machine],[5,crate_tools],[3,crate_items_chainbullets]] call dynamic_crate;
		};	
		if (_randomCrate >= 80 && _randomCrate <= 100) then {
			[_crate,9,5,[10,crate_items_crop_raider]] call dynamic_crate;
		};			
	};
