if(isServer) then {
	...
	[_mission,_position,"Hard","Mining Tower","MainHero",true] call mission_init;
	
	diag_log 		format["WAI: [Mission:[Hero] Mining Tower]: Starting... %1",_position];
...
_baserunover = [_baserunover0,_baserunover1,_baserunover2,_baserunover3,_baserunover4,_baserunover5];
	...
	//Humvee Patrol
[[(_position select 0) + 50, _position select 1, 0],[(_position select 0) - 60, _position select 1, 0],50,2,new_ai_vehicle_random_hero,"Hard","Bandit","Bandit",_mission] call vehicle_patrol;
...
[[[(_position select 0) + 33, (_position select 1) - 21, 0]],"KORD_high_TK_EP1","Easy","Bandit","Bandit",0,2,"Random","Random",_mission] call spawn_static;
	...
	_complete = [
		...
		"MINING TOWER! Climb up the tower and down the other side to get the loot",	// mission announcement
		"Survivors captured the Mining Tower, HOOAH!!",															// mission success
		"Survivors were unable to capture the Mining Tower"														// mission fail
	] call mission_winorfail;

	if(_complete) then {
	[_crate,5,5,[15,crate_items_mining_tower]] call dynamic_crate;
	};	
	
	diag_log format["WAI: [Mission:[Hero] Mining Tower]: Ended at %1",_position];
